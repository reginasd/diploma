...
function createAdmin($request_values){
	...
	if (empty($role)) { array_push($errors, "Выберите тип пользователя");}
	...
	if ($user) { // if user exists
		...
		if ($user['email'] === $email) {
		  array_push($errors, "Данный Email уже зарегистрирован");
		}
	}
...
function getAllSupplyFlowers() {
	global $conn;
	$sql = "SELECT flowers.name, colors.color_name, flower_color.id 
				FROM flowers, colors, flower_color 
				WHERE flower_color.flower_id=flowers.id 
				AND flower_color.color_id=colors.id";
	$result = mysqli_query($conn, $sql);
	$supflowers = mysqli_fetch_all($result, MYSQLI_ASSOC);
	return $supflowers;
}
function createTopic($request_values){
	...
	// validate form
	if (empty($topic_name)) { 
		array_push($errors, "Введите название подборки"); 
	}
	...
	if (mysqli_num_rows($result) > 0) { // if topic exists
		array_push($errors, "Подборка с таким названием уже существует");
	}
	...
	if (count($errors) == 0) {
		...
		$_SESSION['message'] = "Подборка успешно создана";
...
function updateTopic($request_values) {
	...
	if (count($errors) == 0) {
		...
		$_SESSION['message'] = "Подборка успешно обновлена";
		header('location: topics.php');
		exit(0);
	}
}
...
function deleteTopic($topic_id) {
	...
	if (mysqli_query($conn, $sql)) {
		$_SESSION['message'] = "Подборка успешно удалена";
		header("location: topics.php");
		exit(0);
	}
}
